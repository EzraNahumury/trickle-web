...
/**
 * Ferrofluid — adapted from React Bits (TypeScript + Tailwind variant).
 * WebGL fluid-rim shader on a transparent canvas via `ogl`.
 *
 * Local adaptations:
 * - "use client" (Next.js App Router).
 * - Pointer tracking listens on `window` instead of the canvas: we render this
 *   behind the page with pointer-events:none, so canvas-level events never fire.
 */
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    // Window-level tracking: the canvas sits behind the page content with
    // pointer-events disabled, so events would never reach it directly.
